...
namespace negocio
{
    public class PacienteNegocio
    {
        ...
        public List<Paciente> listar(int IDUsuario)
        {
            List<Paciente> listaPacientes = new List<Paciente>();
            AccesoDatos datos = new AccesoDatos();
            ...
            try
            {
                datos.setQuery("select p.IDUsuario, u.Pass, u.NombreUsuario, p.Nombre, p.Apellido, p.Nacimiento, p.Dni, u.EMail, p.Celular, p.Domicilio, p.CodPostal from PACIENTES p inner join Usuarios u on u.ID = p.IDUsuario WHERE IDUsuario = @IDUsuario and u.Estado=1");
